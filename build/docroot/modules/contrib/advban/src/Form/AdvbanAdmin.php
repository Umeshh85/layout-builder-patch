...
class AdvbanAdmin extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state, $ban_id = NULL) {
    ...
    if ($edit_mode) {
      ...
      if (empty($ban_data)) {
        $this->messenger()->addMessage($this->t('Empty ban data for ID = @id', ['@id' => $ban_id]), 'error');
    ...
    $form['expiry_duration'] = [
      ...
      '#type' => 'select',
      '#options' => [AdvbanHelper::ADVBAN_NEVER => $this->t('Never')] + $expiry_durations,
    ...
    $form['actions']['submit'] = [
      '#type' => 'submit',
      '#value' => $edit_mode ? $this->t('Edit') : $this->t('Add'),
    ];
    if ($edit_mode) {
