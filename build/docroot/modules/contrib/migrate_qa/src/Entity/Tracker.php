 ...
 *     "revision" = "/migrate-qa-tracker/{migrate_qa_tracker}/revisions/{tracker_revision}/view",
...
class Tracker extends ContentEntityBase implements TrackerInterface {
  ...
  public function preSave(EntityStorageInterface $storage) {
    ...
    // Conditionally create a revision.
    $create_revision = FALSE;
    // Make a new revision if a log message is set.
    if ($this->getRevisionLogMessage()) {
      $create_revision = TRUE;
    }
    // Make a new revision if there are changes to the tracker.
    if (!$this->isNew() && empty($create_revision)) {
      $id = $this->id();
      $original_entity = $this->entityTypeManager()->getStorage('migrate_qa_tracker')->load($id);
      $original_entity_array = $original_entity->toArray();
      $current_entity_array = $this->toArray();
      $diffs = [];
      $diffs['normal'] = DiffArray::diffAssocRecursive($current_entity_array, $original_entity_array);
      // Swap the arguments to check for removals.
      $diffs['reverse'] = DiffArray::diffAssocRecursive($original_entity_array, $current_entity_array);
      // Ignore properties that shouldn't count as content changes.
      $ignore_properties = ['changed', 'revision_user'];
      foreach ($diffs as $id => $diff) {
        foreach ($ignore_properties as $property) {
          if (isset($diff[$property])) {
            unset($diffs[$id][$property]);
          }
        }
      }
      if (!empty($diffs['normal']) || !empty($diffs['reverse'])) {
        $create_revision = TRUE;
      }
    }
    if ($create_revision) {
