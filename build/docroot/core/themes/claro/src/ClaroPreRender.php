    ...
    if ($group_type_is_details && $groups_are_present) {
      ...
      foreach ($group_keys as $group_key) {
        ...
        foreach ($children_keys as $child_key) {
          ...
          if ($type === 'details') {
            // Mark the details element as vertical tabs item.
            $element['group']['#groups'][$group_key][$child_key]['#vertical_tab_item'] = TRUE;

            if ($first_key) {
              // Mark the first vertical tabs item.
              $element['group']['#groups'][$group_key][$child_key]['#vertical_tab_item_first'] = TRUE;
              $first_key = FALSE;
            }
      ...
      // Add a flag to the last vertical tabs item.
      if ($last_group_with_child_key && $last_group_with_child_key_last_child_key) {
        $element['group']['#groups'][$last_group_with_child_key][$last_group_with_child_key_last_child_key]['#vertical_tab_item_last'] = TRUE;
      }
